...
export class Scheduler {
  ...
  constructor(config: SchedulerConfig = defaultSchedulerConfig) {
    this.config = config;
  }
  ...
  private cleanupExpiredLeases(db: any): number {
    // Find expired leases
  ...
  private markOfflineWorkers(db: any): number {
    const timeoutThreshold = new Date(
      Date.now() - this.config.workerTimeoutMinutes * 60 * 1000
    ).toISOString();
  ...
  private requeueTasksFromOfflineWorkers(db: any): number {
