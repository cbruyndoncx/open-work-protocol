...
export async function adminRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/v1/admin/tasks', async (request, reply) => {
    try {
      ...
      const repository = db.prepare('SELECT id FROM repositories WHERE repo = ?').get(validated.repo) as any;
      if (!repository) {
      ...
      `).run(
        ...
        validated.estimate_points,
        validated.priority,
        JSON.stringify(validated.required_skills),
        validated.area || null,
        validated.tier
      );
